...
##==========================================================================================
        ...
        if options.exist('tracks'):
            # Read tracks
            trackfile = options.getfieldvalue('tracks')
            if np.all(isinstance(trackfile, str)): # if all(isinstance(track, str)):
                track = expread(trackfile)
            else:
                track = float(trackfile)

            for i in range(len(track)):
                trx = np.hstack((track[i]['x'].reshape(-1,1), track[i]['y'].reshape(-1,1)))
                flags = ContourToNodes(track[i]['x'], track[i]['y'], [domain[0]], 0)[0]
                trx = trx[np.nonzero(flags), :][0]

                if np.size(trx, axis=1) == 2:
                    trx = np.hstack((trx, 3. * np.ones((np.size(trx, axis=0), 1)))) # track = np.hstack((track, 3. * np.ones((size(track, axis=0), 1))))
            ...
            # track = track[np.nonzero(flags), :][0] #track[np.nonzero(flags), :]

                # Add all points to bamg_geometry
                nods = np.size(trx, axis=0)
                bamg_geometry.Vertices = np.vstack((bamg_geometry.Vertices, trx))
                bamg_geometry.Edges = np.vstack((
                    bamg_geometry.Edges,
                    np.hstack((
                        np.arange(count+1, count+nods).reshape(-1,1), # np.arange(count + 1, count + nods).reshape(-1, ),
                        np.arange(count+2, count+nods+1).reshape(-1,1), # np.arange(count + 2, count + nods + 1).reshape(-1, ),
                        3. * np.ones((nods-1, 1))
                    ))
                ))

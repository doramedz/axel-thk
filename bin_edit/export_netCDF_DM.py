...
# D Medrzycka
##=========================================
# issue w/ zlib: # RuntimeError: NetCDF: Filter error: bad id or parameters or duplicate filter
# createVariable fails with non-empty lists of strings AND non-empty dicts
# 399: -[zlib = True] +[zlib = False]
# 394: -[zlib = True] +[zlib = False]
#
#
# np.mod doesn't like np.nan: # RuntimeWarning: invalid value encountered in remainder
# 408: +[np.isnan(np.sum(var)) == False and]
#
# lists require np.abs: # TypeError: bad operand type for abs(): 'list'
# 408: -[abs] +[np.abs]
##==========================================================================================
#
        ...
        for field in fields:
            ...
            if type(Var) == list:
                ...
                if StdList:  # this is a standard or empty list just proceed
                    ...
                    Var = SqueezeVar(Var)
                    DimDict, ncvar = CreateVar(NCData, Var, field, NCgroup, DimDict) ######### ln 168
...
##==========================================================================================
# issue w/ zlib
# Error: RuntimeError: NetCDF: Filter error: bad id or parameters or duplicate filter
##==========================================================================================
    # treating list as string table
        ...
        if val_shape in [(), (0,), 0]:
            ...
            dimensions, DimDict = GetDim(NCData, val_shape, val_type, DimDict, val_dim)
            ncvar = Group.createVariable(str(field), nctype, dimensions=dimensions, zlib=False) # zlib=True
    # treating  dict as string tables
        ...
        if val_shape in [(), (0,), 0]:
            ...
            dimensions, DimDict = GetDim(NCData, val_shape, val_type, DimDict, val_dim)
            ncvar = Group.createVariable(str(field), str, dimensions=dimensions, zlib=False) # zlib=True
##==========================================================================================
#
##==========================================================================================
    ...
    # Now dealing with doubles, we convert them to int if possible
        ...
        try:
            ...
            # NO EDIT:: # IsInt = np.sum(np.mod(var, 1)) == 0 and np.all(abs(var) < 2147483647) and len(var) > 0
            # EDIT (DM):: # +[np.isnan(np.sum(var)) == False and], -[abs] +[np.abs]
            IsInt = np.isnan(np.sum(var)) == False and np.sum(np.mod(var, 1)) == 0 and np.all(np.abs(var) < 2147483647) and len(var) > 0
        except TypeError:
            #check if we are integer and under C long overflow
            IsInt = np.mod(var, 1) == 0 and np.abs(var) < 2147483647
        if IsInt:
            val_type = 'int64'
